...
class AdapterCache:
    ...
    def __init__(self, config: CacheConfig):
        ...
        print("📁 Adapter cache initialized:")
    ...
    def get_or_train_adapter(self,
        ...
        if cache_hash in self._cache:
            ...
            if Path(adapter_path).exists():
                print("💾 [CACHE] HIT: Found in-memory cache")
                print(f"   📁 Adapter: {adapter_path}")
                return str(adapter_path)  # 🔥 FIX: Always return string for Modal consistency
            else:
                # Remove stale cache entry
                print("⚠️  [CACHE] STALE: In-memory entry exists but file missing")
        ...
        if adapter_path.exists() and adapter_path.is_dir():
            print("💿 [CACHE] HIT: Found in primary storage")
            print(f"   📁 Adapter: {adapter_path}")
        ...
        if is_modal_environment():
            print("🎯 Modal-native execution: no syncing needed")

        # 🔥 CRITICAL FIX: Try volume reload before declaring cache miss
        if is_modal_environment():
            print("🔄 [CACHE] Attempting Modal volume reload before final cache miss decision...")
            try:
                import modal
                volume = modal.Volume.from_name("coral-x-clean-cache")
                volume.reload()
                print("✅ Modal volume reloaded - checking cache again...")
                ...
                if adapter_path.exists() and adapter_path.is_dir():
                    ...
                    print("❌ [CACHE] Still not found after volume reload - confirmed miss")

            except Exception as reload_error:
                print(f"⚠️  Volume reload warning during cache check: {reload_error}")

        # Train new adapter
        print("❌ [CACHE] MISS: No cached adapter found")
    ...
    def _move_adapter_to_cache(self, source_path: str, target_path: Path) -> None:
        ...
        if source.is_dir():
            ...
            print("📁 Adapter directory moved successfully")
            ...
            print("📄 Adapter file moved successfully")
        ...
        if is_modal_environment():
            try:
                ...
                print("💾 Modal volume sync completed")
    ...
    def _cleanup_legacy_pt_files(self) -> None:
        ...
        for pt_entry in legacy_entries:
            try:
                ...
                if pt_entry.is_dir():
                    ...
                    print("      This causes major confusion - renaming to proper format...")

                    if corresponding_dir.exists():
                        print("      Target directory already exists, removing problematic .pt directory")
                        import shutil
                    ...
                    if corresponding_dir.exists() and corresponding_dir.is_dir():
                        ...
                        print("      Removing to prevent format conflicts...")
                        pt_entry.unlink()
                        removed_count += 1
        ...
        if removed_count > 0:
            ...
            print("   💡 LoRA adapters now use consistent directory format")

    def force_cleanup_legacy_files(self) -> Dict[str, int]:
        """Force cleanup of all legacy .pt files and return statistics."""
        print("🧹 [CACHE] FORCE CLEANUP: Removing all legacy .pt files...")
        ...
        print("✅ [CACHE] Force cleanup completed:")
    ...
    def get_or_create_adapter(self, lora_config: LoRAConfig, genome_id: str = "unknown") -> str:
        ...
        if self.has_adapter(cache_hash):
            ...
            print("   ✅ [CACHE] HIT - Reusing existing adapter")
            ...
            if usage_info['reuse_count'] > 5:
                print("      ⚠️  HIGH CACHE REUSE - Consider if this is expected")
            elif usage_info['reuse_count'] > 1:
                print("      ✅ MODERATE CACHE SHARING - Good efficiency")

            return adapter_path
        else:
            # Cache MISS - need to create new adapter
            print("   ❌ [CACHE] MISS - Creating new adapter")
            print("      • This adapter configuration has not been seen before")

            # Check for similar configurations (debugging cache granularity)
            similar_configs = self._find_similar_configs(lora_config)
            if similar_configs:
                print("      📊 SIMILAR CONFIGURATIONS FOUND:")
                ...
                print("      🔬 UNIQUE CONFIGURATION - No similar configs found")
    ...
    def print_cache_usage_summary(self):
        ...
        if cache_efficiency > 8:
            print("   ⚠️  VERY HIGH CACHE REUSE - May indicate insufficient diversity")
        elif cache_efficiency > 4:
            print("   ✅ HIGH CACHE EFFICIENCY - Good balance")
        elif cache_efficiency > 2:
            print("   ✅ MODERATE CACHE EFFICIENCY - Reasonable sharing")
        else:
            print("   ❌ LOW CACHE EFFICIENCY - Most genomes unique")
        ...
        for i, (cache_hash, info) in enumerate(sorted_groups[:10]):  # Show top 10
            ...
            if reuse_count > 8:
                print("      ❌ EXCESSIVE SHARING - Check if this is expected")
            elif reuse_count > 3:
                print("      ⚠️  HIGH SHARING - Monitor diversity")
            elif reuse_count > 0:
                print("      ✅ MODERATE SHARING - Good efficiency")
            else:
                print("      🔬 UNIQUE ADAPTER - No sharing yet")
        ...
        if overall_diversity < 0.3:
            print("   ❌ LOW DIVERSITY - Parameters too similar")
        elif overall_diversity < 0.6:
            print("   ⚠️  MODERATE DIVERSITY - Room for improvement")
        else:
            print("   ✅ HIGH DIVERSITY - Good parameter spread")
