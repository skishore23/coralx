...
def get_cached_model(model_name: str, torch_dtype: str = "float16", cache_dir: str = "/cache/models"):
    ...
    try:
        ...
        print("📦 Downloading tokenizer to warm cache...")
        tokenizer = AutoTokenizer.from_pretrained(
            model_name,
            cache_dir=cache_dir,
    ...
    try:
        ...
        print("📦 Downloading model to warm cache (~13GB)...")
        try:
            model = AutoModelForCausalLM.from_pretrained(
                model_name,
                torch_dtype=torch_dtype_obj,
...
def smart_volume_reload():
    ...
    try:
        ...
        print("✅ Modal volume reloaded (throttled)")
    except Exception as e:
...
def generate_with_codellama_modal(request: GenerationRequest) -> GenerationResult:
    ...
    try:
        ...
        if free_memory < 4.0:
            ...
            global _model_cache
            _model_cache.clear()
            print("🚨 Emergency cache clear due to low memory")
        ...
        if not isinstance(request.problem_name, str) or not isinstance(request.buggy_code, str):
            raise GenerationError("Invalid request parameters: problem_name and buggy_code must be strings")
        ...
        try:
            ...
            print("🔧 OOM during generation, retrying with minimal settings...")
            clear_cuda()
            ...
            try:
                ...
                print("✅ Generation succeeded with emergency settings")
            except torch.cuda.OutOfMemoryError:
