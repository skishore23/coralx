...
def _calculate_hypervolume(population: Population) -> float:
    """Small test helper for normalized average objective coverage."""
    evaluated = [g for g in population.genomes if g.multi_scores is not None]
    if not evaluated:
        return 0.0
...
class TestEvolutionLoop:
    ...
    def multi_objective_config(self, temp_dir):
        ...
        config_data = {
            ...
            "experiment": {
                ...
                "model": {"name": "mock_model_for_test", "max_seq_length": 256},
            },
            "training": {"batch_size": 1, "epochs": 1, "learning_rate": 2e-4},
            "evaluation": {
                "test_samples": 2,
                "fitness_weights": {
                    "bugfix": 0.3,
    ...
    def test_hypervolume_calculation(self):
        ...
        hv = _calculate_hypervolume(population)
    ...
    async def test_multi_generation_evolution(self, temp_dir, multi_objective_config):
        ...
        try:
            ...
            # assert (results_dir / "evolution.jsonl").exists()
            ...
            assert (
                progress_data["current_generation"] >= 1
            )  # At least 1 generation completed
    ...
    def test_monotone_improvement_or_diversity(self, temp_dir):
        ...
        gen0_hv = _calculate_hypervolume(
            Population(
                tuple(
                    [
        ...
        gen1_hv = _calculate_hypervolume(
            Population(
                tuple(
                    [
        ...
        gen1_objectives = [
            [s.bugfix, s.style, s.security, s.runtime, s.syntax] for s in gen1_scores
        ]

        # Calculate variance across objectives
        import numpy as np

        variances = [np.var([obj[i] for obj in gen1_objectives]) for i in range(5)]
