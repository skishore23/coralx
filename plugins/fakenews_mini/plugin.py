...
class FakeNewsMiniDataset(DatasetProvider):
    ...
    def problems(self) -> Iterable[dict[str, Any]]:
        ...
        yield from FAKENEWS_MINI_SAMPLES


class FakeNewsMiniRunner(ModelRunner):
    """Mini FakeNews model runner for M1 testing."""
    ...
    def __init__(
        ...
        print(
            f"FakeNews Mini runner initialized for genome {genome.id if genome else 'unknown'}"
        )
    ...
    def generate(self, prompt: str, max_tokens: int = 10, cheap_knobs=None) -> str:
        ...
        if fake_score > real_score:
            ...
            if len(text) > 100 and any(
                word in text for word in ["discover", "confirm", "admit"]
            ):
                return "fake"
            else:
                return "real"
    ...
    def _setup_model(self):
        ...
        print("   Setting up FakeNews Mini model...")
        print(
...
class FakeNewsMiniFitness(FitnessFn):
    ...
    def evaluate_multi_objective(
        ...
        for i, problem in enumerate(problems_list, 1):
            ...
            try:
                ...
                print(
                    f"   {status} Predicted: {'fake' if pred_label == 1 else 'real'} (conf: {confidence:.2f})"
                )
    ...
    def _calculate_auroc(
        self, predictions: list[int], ground_truth: list[int], confidences: list[float]
    ) -> float:
        ...
        sorted_data = sorted(
            zip(confidences, predictions, ground_truth, strict=False), reverse=True
        )

        tp = fp = tn = fn = 0
        auroc = 0.0

        for _conf, pred, truth in sorted_data:
...
class FakeNewsMiniPlugin:
    ...
    def model_factory(self) -> Callable[[LoRAConfig, Genome | None], ModelRunner]:
        ...
        def create_model(
            ...
            return FakeNewsMiniRunner(lora_cfg, self.config, genome=genome)
