###############################################################################
# Minimal NEAT‑style evolutionary engine (pure except RNG).
###############################################################################
...
def mutate(genome: Genome, evo_cfg: EvolutionConfig, rng: Random, generation: int = 0, 
    ...
    if rng.random() < 0.7:
        ...
        # FAIL-FAST: config_dict is required for proper CA → LoRA mapping
        if not config_dict:
            raise ValueError(
                f"FAIL-FAST: config_dict required for CA mutation. "
                f"Cannot perform proper CA → LoRA mapping without configuration. "
                f"No fallback to original LoRA config allowed."
            )
...
def crossover(p1: Genome, p2: Genome, evo_cfg: EvolutionConfig, rng: Random, generation: int = 0,
    ...
    if not config_dict:
        raise ValueError(
            f"FAIL-FAST: config_dict required for crossover. "
            f"Cannot perform proper CA → LoRA mapping without configuration. "
            f"No fallback to direct LoRA crossover allowed."
        )
