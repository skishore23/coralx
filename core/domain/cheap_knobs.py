"""
Runtime generation parameter control from cellular automata features.

This module implements the second optimization loop - controlling model
inference parameters (temperature, top_k, etc.) based on CA features.
These parameters are adjusted at runtime without requiring model retraining.
"""
...
def map_ca_features_to_cheap_knobs(
    features: CAFeatures, base_config: dict[str, Any] = None
) -> CheapKnobs:
    """
    ...
    if base_config is None:
        raise ValueError(
            "  base_config required for cheap knobs mapping - no defaults provided"
        )
    ...
    for range_key in required_ranges:
        if range_key not in base_config:
            raise ValueError(
                f"  {range_key} missing from cheap knobs config - two-loop architecture requires explicit ranges"
            )
    ...
    repetition_penalty = _map_periodicity_to_repetition_penalty(
        features.periodicity, rep_penalty_range
    )
...
def _map_periodicity_to_repetition_penalty(
    periodicity: float, penalty_range: tuple
) -> float:
    """
...
def cheap_knobs_to_generation_kwargs(knobs: CheapKnobs) -> dict[str, Any]:
    """
    Convert CheapKnobs to generic generation keyword arguments.
