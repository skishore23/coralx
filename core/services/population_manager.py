...
class PopulationManager(LoggingMixin):
    ...
    def __init__(self, config: CoralConfig, random_seed: int | None = None):
        ...
        self.logger.info(
            f"Population manager initialized: size={self.execution_config.population_size}, mode={self.execution_config.selection_mode}, survival_rate={self.execution_config.survival_rate}"
        )
    ...
    def validate_population(self, population: Population) -> None:
        ...
        if population.size() > self.execution_config.population_size * 2:
            self.logger.warning(
                f"Population size exceeded: current_size={population.size()}, max_size={self.execution_config.population_size * 2}"
            )
    ...
    def apply_threshold_gate(
        ...
        """
        self.logger.info(
            f"Applying threshold gate: generation={generation}, population_size={population.size()}"
        )

        # Calculate dynamic thresholds for current generation
        dynamic_thresholds = calculate_dynamic_thresholds(
            generation, self.execution_config.generations, self.threshold_config
        )
        ...
        filtered_genomes = filter_population_by_thresholds(
            list(population.genomes), score_extractor, dynamic_thresholds
        )
        ...
        self.logger.info(
            f"Threshold gate applied: generation={generation}, original_size={population.size()}, passed_count={passed_count}, filtered_count={filtered_count}, pass_rate={passed_count / population.size() if population.size() > 0 else 0}"
        )

        # Ensure minimum population size
        if passed_count < 2:
            self.logger.warning(
                f"Population too small after filtering: remaining_genomes={passed_count}, generation={generation}"
            )
            ...
            if population.size() >= 2:
                ...
                sorted_genomes = sorted(
                    population.genomes, key=lambda g: g.fitness, reverse=True
                )
    ...
    def select_survivors(self, population: Population) -> Population:
        ...
        self.logger.info(
            f"Selecting survivors: population_size={population.size()}, survival_rate={survival_rate}, num_survivors={num_survivors}, selection_mode={self.execution_config.selection_mode}"
        )
        ...
        if self.execution_config.selection_mode.value == "pareto":
            ...
            survivors = tournament_select(
                population, num_survivors, tournament_size=3, rng=self.random
            )
        else:
        ...
        self.logger.info(
            f"Survivors selected: selected_count={survivors.size()}, selection_mode={self.execution_config.selection_mode}"
        )
    ...
    def calculate_diversity_metrics(self, population: Population) -> dict[str, float]:
        ...
        if len(fitness_scores) > 1:
            ...
            fitness_variance = sum(
                (f - mean_fitness) ** 2 for f in fitness_scores
            ) / len(fitness_scores)
                ...
                for j in range(i + 1, len(phenotype_vectors)):
                    dist = (
                        sum(
                            (a - b) ** 2
                            for a, b in zip(
                                phenotype_vectors[i], phenotype_vectors[j], strict=False
                            )
                        )
                        ** 0.5
                    )
    ...
    def should_stop_early(
        ...
        diversity_decline = (
            len([d for d in recent_diversity if d < 0.1]) >= plateau_window // 2
        )
        ...
        if should_stop:
            self.logger.info(
                f"Early stopping triggered: generation={generation}, fitness_improvement={fitness_improvement}, min_improvement={min_improvement}, diversity_decline={diversity_decline}"
            )
    ...
    def record_generation_stats(self, population: Population) -> None:
        ...
        self.logger.info(
            f"Generation stats recorded: generation={self.current_generation}, population_size={population.size()}, best_fitness={best_fitness}, avg_diversity={avg_diversity}"
        )
