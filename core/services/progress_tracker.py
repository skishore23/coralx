...
class ProgressTracker(LoggingMixin):
    ...
    def __init__(self, config: CoralConfig, run_id: str | None = None):
        super().__init__()
        self.config = config
        ...
        self.logger.info(
            f"Progress tracker initialized: run_id={self.run_id}, file={self.progress_file_path}, max_generations={self.max_generations}"
        )
    ...
    def update_status(
        self, status: str, message: str, best_genome: Genome | None = None
    ) -> None:
        ...
        self.logger.info(
            f"Progress updated: status={status}, generation={self.current_generation}, message='{message}', elapsed_time={elapsed_time:.2f}s"
        )
    ...
    def update_generation_progress(
        ...
        evaluation_rate = (
            evaluated_count / population.size() if population.size() > 0 else 0.0
        )
        ...
        if len(existing_progress["generation_history"]) > 50:
            existing_progress["generation_history"] = existing_progress[
                "generation_history"
            ][-50:]
        ...
        self.logger.info(
            f"Generation progress updated: generation={generation}, population_size={population.size()}, evaluated_count={evaluated_count}, diversity_score={diversity_score}"
        )

    def update_cache_stats(
        self, hit_rate: float, total_adapters: int, cache_size_mb: float
    ) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"Cache stats updated: hit_rate={hit_rate}, total_adapters={total_adapters}, cache_size_mb={cache_size_mb}"
            )
        except Exception as e:
            self.logger.error(f"Cache stats update failed: {e}")

    def update_training_stats(
        self, adapters_trained: int, training_rate: float, current_adapter: str
    ) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"Training stats updated: adapters_trained={adapters_trained}, training_rate={training_rate}, current_adapter={current_adapter}"
            )
        except Exception as e:
    ...
    def _save_progress(self, progress_data: dict[str, Any]) -> None:
        ...
        try:
            ...
            raise InfrastructureError(
                f"Failed to save progress file: {e}",
                context={"progress_file": str(self.progress_file_path)},
            )
    ...
    def cleanup_old_progress_files(self, max_files: int = 10) -> None:
        ...
        try:
            ...
            if len(progress_files) > max_files:
                ...
                self.logger.info(
                    f"Old progress files cleaned: removed_count={len(files_to_remove)}, remaining_count={len(progress_files) - len(files_to_remove)}"
                )
        except Exception as e:
