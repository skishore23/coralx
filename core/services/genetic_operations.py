...
from ..domain.neat import Population, crossover, mutate
...
class GeneticOperationsService(LoggingMixin):
    ...
    def __init__(self, config: CoralConfig, random_seed: int | None = None):
        super().__init__()
        self.config = config
        ...
        self.genetic_tracker = GeneticOperationsTracker(
            output_dir=str(genetic_tracking_dir)
        )
        ...
        self.logger.info(
            f"Genetic operations service initialized: crossover_rate={self.crossover_rate}, population_size={self.population_size}"
        )

    def reproduce_population(
        self, survivors: Population, target_size: int, generation: int
    ) -> Population:
        ...
        if survivors.size() == 0:
            raise GeneticOperationError(
                "Cannot reproduce from empty survivor population"
            )

        self.logger.info(
            f"Reproduction started: generation={generation}, survivors_count={survivors.size()}, target_size={target_size}, crossover_rate={self.crossover_rate}"
        )
        ...
        if offspring_needed <= 0:
            self.logger.info(
                f"No offspring needed: target_size={target_size}, survivors={survivors.size()}"
            )
            return survivors
        ...
        self._record_generation_stats(
            generation, crossovers_performed, mutations_performed, operation_time
        )

        result_population = Population(tuple(new_genomes))

        self.logger.info(
            f"Reproduction completed: generation={generation}, final_size={result_population.size()}, crossovers={crossovers_performed}, mutations={mutations_performed}, operation_time={operation_time}"
        )
    ...
    def calculate_diversity_strength(self, population: Population) -> float:
        ...
        if len(fitness_scores) <= 1:
            ...
            variance = sum((f - mean_fitness) ** 2 for f in fitness_scores) / len(
                fitness_scores
            )
        ...
        diversity_strength = max(
            0.5, min(2.0, diversity_strength)
        )  # Clamp to reasonable range

        self.logger.debug(
            f"Diversity strength calculated: fitness_diversity={fitness_diversity}, genetic_diversity={genetic_diversity}, diversity_strength={diversity_strength}"
        )

        return diversity_strength

    def adjust_genetic_parameters(
        ...
        if diversity_strength < 0.8:
            ...
            self.logger.info(
                f"Increasing crossover rate due to low diversity: generation={generation}, old_rate={base_crossover}, new_rate={adjusted_crossover}"
            )
        elif diversity_strength > 1.5:
            adjusted_crossover = max(0.3, base_crossover * 0.8)
            self.logger.info(
                f"Decreasing crossover rate due to high diversity: generation={generation}, old_rate={base_crossover}, new_rate={adjusted_crossover}"
            )
        else:
    ...
    def _record_generation_stats(
        self, generation: int, crossovers: int, mutations: int, operation_time: float
    ) -> None:
        ...
        self.logger.info(
            f"Generation genetic stats: generation={generation}, crossovers={crossovers}, mutations={mutations}, operation_time={operation_time}, crossover_rate={self.crossover_rate}"
        )
    ...
    def process_genetic_tracking(self, generation: int) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Genetic tracking failed: generation={generation}, error={str(e)}"
            )
    ...
    def get_generation_summary(self) -> dict[str, Any]:
        ...
        avg_operation_time = sum(self.generation_stats["operation_times"]) / len(
            self.generation_stats["operation_times"]
        )
        avg_diversity_strength = (
            sum(self.generation_stats["diversity_strengths"])
            / len(self.generation_stats["diversity_strengths"])
            if self.generation_stats["diversity_strengths"]
            else 1.0
        )
