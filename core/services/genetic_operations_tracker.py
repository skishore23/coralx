...
class CrossoverRecord:
    ...
    parent1_fitness: float | None = None
    parent2_fitness: float | None = None
    child_fitness: float | None = None
    ca_crossover_details: dict[str, Any] | None = None  # Which parent contributed what
    diversity_strength: float = 1.0

    # Success metrics (filled in after evaluation)
    success_score: float | None = None
    outperformed_parents: bool | None = None
...
class MutationRecord:
    ...
    child_fitness: float | None = None
    ca_mutations: dict[str, Any] | None = None  # Grid, rule, steps changes
    lora_mutations: dict[str, Any] | None = None  # Rank, alpha, dropout changes
    diversity_strength: float = 1.0

    # Success metrics (filled in after evaluation)
    success_score: float | None = None
    improvement_over_parent: float | None = None
...
class GeneticOperationsTracker:
    ...
    def __init__(self, output_dir: str = "results/genetic_tracking"):
        ...
        # Statistics
        self.generation_stats: dict[int, dict[str, Any]] = defaultdict(
            lambda: {
                "crossovers": 0,
                "mutations": 0,
                "ca_mutations": 0,
        ...
        print("🧬 Genetic Operations Tracker initialized")
    ...
    def track_crossover(
        ...
        generation: int,
        diversity_strength: float = 1.0,
        ca_crossover_details: dict[str, Any] = None,
    ) -> CrossoverRecord:
        ...
        print(
            f"   🧬 CA Hybrid: complexity={child_ca_features.get('complexity', 0):.3f}"
        )
    ...
    def track_mutation(
        ...
        if mutation_type == "ca_mutation":
            ...
            lora_mutations = self._analyze_lora_mutations(
                parent.lora_cfg, child.lora_cfg
            )
    ...
    def update_fitness_outcomes(
        self, genome_id: str, fitness: float, multi_scores: dict[str, float] = None
    ):
        ...
        for i, record in enumerate(self.crossover_records):
            if record.child_id == genome_id and record.child_fitness is None:
                ...
                parent_fitnesses = [
                    f
                    for f in [record.parent1_fitness, record.parent2_fitness]
                    if f is not None
                ]
                avg_parent_fitness = (
                ...
                outperformed = (
                    fitness > avg_parent_fitness if parent_fitnesses else None
                )
                success_score = (
                    fitness / avg_parent_fitness if avg_parent_fitness > 0 else 1.0
                )
                ...
                if outperformed:
                    self.generation_stats[record.generation][
                        "successful_crossovers"
                    ] += 1
                    print(
                        f"✅ SUCCESSFUL CROSSOVER: {genome_id} (fitness: {fitness:.3f}, improvement: {success_score:.2f}x)"
                    )
        ...
        for i, record in enumerate(self.mutation_records):
            if record.child_id == genome_id and record.child_fitness is None:
                # Calculate success metrics
                improvement = (
                    fitness - record.parent_fitness
                    if record.parent_fitness is not None
                    else 0.0
                )
                success_score = (
                    fitness / record.parent_fitness
                    if record.parent_fitness and record.parent_fitness > 0
                    else 1.0
                )
                ...
                if improvement > 0:
                    ...
                    print(
                        f"✅ SUCCESSFUL MUTATION: {genome_id} (fitness: {fitness:.3f}, improvement: +{improvement:.3f})"
                    )
    ...
    def detect_genetic_patterns(self, generation: int) -> list[GeneticPattern]:
        ...
        successful_crossovers = [
            r
            for r in self.crossover_records
        ...
        if len(successful_crossovers) >= 3:
            ...
            complexity_combinations = [
                (
                    r.parent1_ca_features.get("complexity", 0),
            ...
            high_low_crosses = sum(
                1 for c1, c2 in complexity_combinations if abs(c1 - c2) > 0.4
            )
                    ...
                    GeneticPattern(
                        ...
                        evidence={
                            ...
                            "avg_success_score": sum(
                                r.success_score for r in successful_crossovers
                            )
                            / len(successful_crossovers),
                        },
        ...
        successful_mutations = [
            r
            for r in self.mutation_records
            if r.generation <= generation
            and r.improvement_over_parent
            and r.improvement_over_parent > 0
        ]

        if len(successful_mutations) >= 5:
            ca_success_rate = sum(
                1 for r in successful_mutations if r.mutation_type == "ca_mutation"
            ) / len(successful_mutations)
                ...
                patterns.append(
                    GeneticPattern(
                        ...
                        evidence={
                            ...
                            "ca_mutations": sum(
                                1
                                for r in successful_mutations
                                if r.mutation_type == "ca_mutation"
                            ),
                            "avg_improvement": sum(
                                r.improvement_over_parent for r in successful_mutations
                            )
                            / len(successful_mutations),
                        },
        ...
        recent_crossovers = [
            r for r in self.crossover_records if r.generation >= generation - 2
        ]
        if len(recent_crossovers) >= 5:
            success_rate = sum(
                1 for r in recent_crossovers if r.outperformed_parents
            ) / len(recent_crossovers)
    ...
    def save_tracking_data(self, generation: int):
        ...
        crossover_file = (
            self.output_dir / f"crossovers_gen{generation}_{timestamp}.json"
        )
        ...
        patterns_file = (
            self.output_dir / f"genetic_patterns_gen{generation}_{timestamp}.json"
        )
    ...
    def get_generation_summary(self, generation: int) -> dict[str, Any]:
        ...
        crossover_success_rate = (
            stats["successful_crossovers"] / max(1, stats["crossovers"])
        ) * 100
        mutation_success_rate = (
            stats["successful_mutations"] / max(1, stats["mutations"])
        ) * 100
        ...
        return {
            ...
            "patterns_detected": len(
                [
                    ...
                    if p.generation_detected == generation
                ]
            ),
        }
    ...
    def _analyze_ca_mutations(
        self, parent_seed: CASeed, child_seed: CASeed
    ) -> dict[str, Any]:
        ...
        if not np.array_equal(parent_seed.grid, child_seed.grid):
            ...
            changes["grid_mutations"] = (
                len(diff_positions[0]) if len(diff_positions) > 0 else 0
            )
    ...
    def _analyze_lora_mutations(
        self, parent_cfg: LoRAConfig, child_cfg: LoRAConfig
    ) -> dict[str, Any]:
        ...
        if parent_cfg.dropout != child_cfg.dropout:
            changes["dropout_change"] = {
                "from": parent_cfg.dropout,
                "to": child_cfg.dropout,
            }
